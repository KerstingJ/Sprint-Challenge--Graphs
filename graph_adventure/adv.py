...
def build_trav_graph(player):
    # Build out empty traversal Graph
    # TODO: this could probably be done on the fly
    stack = []
...
def search_for_unopened_door(traversal_graph, start):
    # breadth first search
    visited = set()

    # using a list as a queue for sake of not building a linked list from scratch
    queue = [[start]]
    ...
    while len(queue) > 0:
        ...
        if room not in visited:
            ...
            # look at the doors in the room, build list of any unopened
            doors = [k for k, v in traversal_graph[room].items() if v == '?']
            # if we have any unopened doors
            if len(doors) > 0:
                # return path to unopened door
                return path[1:]

            # add rooms we've seen before
            for d in traversal_graph[room]:
                queue.append(path + [traversal_graph[room][d]])

    return None


# Depth First Traversal, until I hit a dead end. Then Breadth First Traversal, until i find an unopened door
# This is just a helper
def find_route(priority=list('nwes')):
    opposite = {
        'n': 's',
        's': 'n',
        'e': 'w',
    ...
    traversal_graph = build_trav_graph(player)
    next_move = [None]
    moves = []
    ...
    # while we have not visited alll the rooms
    while len(visited) < len(traversal_graph):
        # get the path and the last move from the stack
        move = next_move.pop()
        ...
        # if we've moved
        if move is not None:
            # add the previous room too this rooms visited rooms
            # print(path)
            traversal_graph[room][opposite[move]] = path[-2]
            # add this room to the previous rooms visited rooms
            traversal_graph[path[-2]][move] = room
            moves.append(move)

        # if we have unvisited directions
        if '?' in traversal_graph[room].values():
            # If we have an unopened door, lets add a door to our next_move
            # print("We have unexplored rooms")

            # Get directions that are unexplored

            unexplored = [d for d, v in traversal_graph[room].items() if v == '?']
            # decide next move based on directions of priority
            for d in priority:
                if d in unexplored:
                    # print(d)
                    # add our next move info to our next move
                    next_move.append(d)
                    path.append(roomGraph[room][1][d])
                    # break out of the for loop we are in when we find a move
                    break

        # if we've gotten to a dead end
        else:
            # print("Hit a dead end")
            # find path to room with unopened door
            # path_back is a list of room id's
            path_back = search_for_unopened_door(traversal_graph, room)
            ...
            for room_id in path_back:
                # find what direction this room is relative to current location
                direction = None
                for d in traversal_graph[room].keys():
                    if traversal_graph[room][d] == room_id:
                        ...
                        break
                room = room_id
                # move that direction and add room id and dir to lists
